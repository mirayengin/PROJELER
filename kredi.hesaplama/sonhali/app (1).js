...
const faiz = () => { // kredi tipine göre faiz oranı nı belirledi
...
const hesapla = () => {
  ...
  let taksitTutari;
  taksitTutari = tutar.value * ((faizim * (Math.pow( 1+faizim ,vade.value  ))) / ((Math.pow(1+faizim ,vade.value  ))-1))
   ...
   document.getElementById("taksitTutari").innerText = virgul(taksitTutari.toFixed(2));
   document.getElementById("genelToplam").innerText = virgul((taksitTutari * vade.value ).toFixed(2)) ;
